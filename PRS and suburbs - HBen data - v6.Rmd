...
Make second figure for blog showing change in nos of HB hhds by inner city vs rest - ten largest cities.
...
# # check calcs
# df_hb11 %>% 
#   filter(ttwa11nm %in% names_10 & 
#            (year == 2011 | year == 2019) & 
#            tenure == "private") %>% 
#   group_by(ttwa11nm, ttwa_popln, year) %>% 
#   mutate(decile = trunc(10 * rank_dist2/((max(rank_dist2) + 1))) + 1) %>% 
#   mutate(inner = factor((decile <= 2), 
#                         levels = c(TRUE, FALSE), 
#                         labels = c('Inner city', 'Rest'))) %>% 
#   group_by(ttwa11nm, ttwa_popln, year, inner) %>% 
#   summarise(claimants = sum(claimants, na.rm = TRUE)) %>% 
#   pivot_wider(names_from = 'year', names_prefix = 'yr', values_from = 'claimants') %>% 
#   mutate(change = 100 * (yr2019/yr2011 - 1), 
#          change_n = yr2019 - yr2011) %>% 
#   select(-yr2011, -yr2019)
...
df_hb11 %>% 
  ...
  labs(# title = 'Percent change in HB households in the PRS - 2011 to 2019\n',
       x = '', 
       y = '\nPecent change in HB households in PRS',
       fill = '')

ggsave(here("figs", "fig blog 2.jpg"), 
       width = 15, height = 10, units = 'cm', dpi = 300)

```

Make calc for counterfactual - no. PRS hhlds missing from inner city - ten largest cities.

```{r blog fig2}

# change in HB for outer
temp <- df_hb11 %>%
  filter(ttwa11nm %in% names_10 &
           (year == 2011 | year == 2019) &
           tenure == "private") %>%
  ...
  summarise(claimants = sum(claimants, na.rm = TRUE)) %>%
  pivot_wider(names_from = 'year', names_prefix = 'yr', values_from = 'claimants') %>%
  mutate(change_rest = 100 * (yr2019/yr2011 - 1)) %>% 
  arrange(desc(ttwa_popln)) %>% 
  filter(inner == 'Rest') %>% 
  ungroup() %>% 
  select(ttwa11nm, change_rest)

# change in HB for inner if matched rate of change for outer  
(temp1 <- df_hb11 %>%
  filter(ttwa11nm %in% names_10 &
           (year == 2011 | year == 2019) &
           tenure == "private") %>%
  ...
  summarise(claimants = sum(claimants, na.rm = TRUE)) %>%
  pivot_wider(names_from = 'year', names_prefix = 'yr', values_from = 'claimants') %>%
  mutate(change_obs = 100 * (yr2019/yr2011 - 1), 
         change_obs_n = round((yr2019 - yr2011),0)) %>% 
  arrange(desc(ttwa_popln)) %>% 
  select(-ttwa_popln) %>% 
  filter(inner == 'Inner city') %>% 
  left_join(temp, by = 'ttwa11nm') %>% 
  mutate(change_pred_n = round(yr2011 * (change_rest/100),0), 
         change_diff_n = change_obs_n - change_pred_n,
         change_diff = 100 * change_diff_n/yr2011))

```

Total number HB hhlds missing from inner cities
