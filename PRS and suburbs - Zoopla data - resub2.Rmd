---
title: "PRS and Suburbanisation of Poverty - Zoopla data - resubmission 2"
author: "Nick Bailey"
date: '2023-08-11'
...
# 10/20/30th centile rent for TTWA
df_z74 <- df_z74 %>% 
  group_by(ttwa, year_fiscal) %>% 
  mutate(low20 = (price <= quantile(price, .2))) %>% 
  ungroup()
...
df_z74_decile <- df_z74 %>% 
  ...
  summarise(count = n(), 
            lcount = sum(lowlha == 1), 
            acount = sum(lowlha == 0), 
            l20count = sum(low20 == 1), 
            a20count = sum(low20 == 0), 
...
df_z74_rci <- df_z74_decile %>%
  ...
  mutate(above_pct = acount/sum(acount),
         ...
         above20_pct = a20count/sum(a20count),
         below20_pct= l20count/sum(l20count),
         above20_cum  = cumsum(above20_pct),
         below20_cum = cumsum(below20_pct),
         above20_cum_lag  = lag(above20_cum),
         below20_cum_lag = lag(below20_cum)) %>% 
  mutate(rci = (below_cum_lag * above_cum) - (above_cum_lag * below_cum), 
         rci20 = (below20_cum_lag * above20_cum) - (above20_cum_lag * below20_cum))  %>% 
  summarise(rci = sum(rci, na.rm = TRUE), 
            rci20 = sum(rci20, na.rm = TRUE), 
            ttwa_popln = mean(ttwa_popln))
...
df_z74_decile %>% 
  ...
  labs(title = "Properties above/below LHA by decile - all years")
...
df_z74_decile %>% 
  ...
  facet_wrap(~ ttwa_size) +
  labs(title = 'Share of ads below LHA by decile, year')

```

Share above/below 20th centile for TTWA within each decile.
...
df_z74_decile %>% 
  ...
  facet_wrap(~ ttwa_size) +
  labs(title = 'Share below 20th centile rent for TTWA by decile, year')
...
# biggest 10 ex. slough/heathrow
names_10 <- df_ttwa %>% 
  select(ttwa11nm, ttwa_popln) %>% 
  arrange(desc(ttwa_popln)) %>% 
  filter(ttwa11nm != 'Slough and Heathrow') %>% 
  pull(ttwa11nm)
names_10 <- names_10[1:10]
names_10

# two beds is largest category
temp <- df_z74 %>% 
  filter(beds == 2 & 
           (year_fiscal == 2012|year_fiscal == 2019) & 
           ttwa11nm %in% names_10) %>% 
  mutate(inner = factor((decile <= 2), 
                        levels = c(FALSE, TRUE), 
                        labels = c('outer', 'inner'))) %>% 
  group_by(ttwa, ttwa11nm, ttwa_popln, ttwa_size, year_fiscal, inner) %>%
  summarise(price = median(price, na.rm = TRUE)) %>% 
  ungroup() %>% 
  pivot_wider(names_from = 'year_fiscal', names_prefix = 'yr', values_from = 'price') %>% 
  mutate(change = 100 * (yr2019/yr2012 - 1)) %>% 
  select(-yr2012, -yr2019) %>% 
  arrange(desc(ttwa_popln)) %>% 
  pivot_wider(names_from = 'inner', values_from = 'change') %>% 
  select(ttwa11nm, inner, outer)
...
df_z74 %>% 
  ...
  select(-yr2012, -yr2019) %>% 
  arrange(desc(ttwa_popln)) %>% 
  ggplot(aes(x = reorder(ttwa11nm, ttwa_popln), y = change, fill = inner)) +
  geom_bar(position = 'dodge', stat = 'identity') + 
  coord_flip() +
  theme_minimal(base_size = 14) + 
  scale_fill_manual(values = cbPalette)+
  labs(# title = 'Rent increases for inner city versus rest - 2012 to 2019\n',
       x = '', 
       y = '\nPecent increase in rents',
       fill = '')

ggsave(here("suburbs paper", "figs", "fig blog.jpg"), 
       width = 15, height = 10, units = 'cm', dpi = 300)

# 'Note: Inner city - most central fifth. Data is Zoopla Plc, © 2018, processed by UBDC, University of Glasgow'
...
RCI below LHA vs rest - 2012+13 vs 2018+19 (two year)
...
#   mutate(rci_1213 = (rci2012 + rci2013)/2,
#          rci_1819 = (rci2018 + rci2019)/2)  %>%
#   left_join((df_ttwa %>% select(ttwa11cd, ttwa11nm)), by = c('ttwa' = 'ttwa11cd')) %>%
#   select(ttwa, ttwa11nm, ttwa_size, ttwa_popln, rci_1213, rci_1819) %>%
#   arrange(desc(ttwa_popln))
...
#        y = "RCI 2018-20\n", 
#        shape = "TTWA size") +
#   geom_label(x = -.4, y = .45, label = "Affordable listings \nmore centralised") +
#   geom_label(x = .35, y = -.45, label = "Affordable listings \nmore decentralised")


# this version of fig3 dropped from final submission
df_z74_rci %>% 
  select(-rci20) %>% 
  pivot_wider(names_from = year_fiscal, names_prefix = "rci",
              values_from = rci) %>% 
  mutate(rci_1213 = (rci2012 + rci2013)/2, 
         rci_1819 = (rci2018 + rci2019)/2) %>% 
  mutate(ttwa_size = factor(ttwa_size, 
                            levels = c('1500k+', '750-1500k', '250-750k'))) %>%  
  ggplot(aes(x = rci_1213, y = rci_1819, group = ttwa_size)) +
...
RCI below 20th centile vs rest - 2012+13 vs 2018+19 (two year)

```{r rci20 start vs end}
...
df_z74_rci %>% 
  ...
  labs(#title = "RCI start vs RCI end", 
       ...
       y = "RCI 2018-20\n", 
       shape = "TTWA size") +
  geom_label(x = -.3, y = .4, label = "Affordable listings \nmore centralised") +
  geom_label(x = .3, y = -.4, label = "Affordable listings \nmore decentralised")

# ggsave(here("suburbs paper", "figs", "Fig 3 20th centile.jpg"), 
#        width = 20, height = 15, units = "cm", dpi = 600)

```

Both LHA and 20th centile - 2012+13 vs 2018+19 (two year)

```{r rci20 start vs end, width = 12, height = 5}

# # data for plot
# df_z74_rci %>%
#   pivot_longer(cols = c(rci, rci20), names_to = 'rci_type', values_to = 'rci_value') %>%
#   pivot_wider(names_from = year_fiscal, names_prefix = "rci",
#               values_from = rci_value) %>%
#   mutate(rci_1213 = (rci2012 + rci2013)/2,
#          rci_1819 = (rci2018 + rci2019)/2)  %>%
#   left_join((df_ttwa %>% select(ttwa11cd, ttwa11nm)), by = c('ttwa' = 'ttwa11cd')) %>%
#   select(ttwa, ttwa11nm, ttwa_size, ttwa_popln, rci_type, rci_1213, rci_1819) %>%
#   arrange(desc(ttwa_popln))


# alt version of fig3 in the paper
df_z74_rci %>% 
  pivot_longer(cols = c(rci, rci20), names_to = 'rci_type', values_to = 'rci_value') %>%
  pivot_wider(names_from = year_fiscal, names_prefix = "rci",
              values_from = rci_value) %>%
  ...
  mutate(ttwa_size = factor(ttwa_size, 
                            levels = c('1500k+', '750-1500k', '250-750k')), 
         rci_type = factor(rci_type, 
                           levels = c('rci', 'rci20'), 
                           labels = c('a. Below HB rate', 'b. Below 20th centile'))) %>%  
  mutate(ttwa_name = case_when(ttwa == 'E30000234' ~ 'London', 
                               TRUE ~ NA)) %>% 
  ggplot(aes(x = rci_1213, y = rci_1819, group = ttwa_size, label = ttwa_name)) +
  geom_point(aes(shape = ttwa_size), colour = "grey30", size=2.5) + 
  geom_text_repel(nudge_x = -.2, nudge_y = .1) +
  coord_cartesian(xlim = c(-.6, .5),
                  ylim = c(-.6, .5)) +
  scale_shape_manual(values=c(15, 12, 0))+
  geom_hline(yintercept = 0, colour = "grey50") +
  geom_vline(xintercept = 0, colour = "grey50") +
  geom_abline(intercept = 0, slope = 1, colour = "grey40", linetype = "dashed") +
  facet_wrap(~ rci_type) +
  ...
  labs(#title = "RCI start vs RCI end", 
       ...
       y = "RCI 2018-20\n", 
       shape = "TTWA size") +
  geom_label(x = -.35, y = .45, label = "Affordable \nmore central", size = 3.5) +
  geom_label(x = .35, y = -.45, label = "Affordable \nless central", size = 3.5)
